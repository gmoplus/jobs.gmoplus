...
<form onsubmit="return submitHandler();" action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;field={$smarty.get.field}{/if}" method="post">
    ...
    <div id="field_text" class="hide">
        ...
        {if $allLangs|@count > 1 && $smarty.get.field != 'First_name' && $smarty.get.field != 'Last_name' && $smarty.get.field != 'keyword_search'}
        <div id="text_multilingual" {if $text_cond.condition}class="hide"{/if}>
            ...
            <div id="text_translate" {if !$sPost.text.multilingual}class="hide"{/if}>
                <table class="form">
                <tr>
                    <td class="name">{$lang.translate_text}</td>
                    <td class="field">
                        {if $sPost.text.translate == '1'}
                            {assign var='text_translate_yes' value='checked="checked"'}
                        {else}
                            {assign var='text_translate_no' value='checked="checked"'}
                        {/if}

                        <label><input {$text_translate_yes} type="radio" name="text[translate]" value="1" {if !$_isTranslatorConfigured}disabled="disabled"{/if} /> {$lang.yes}</label>
                        <label><input {$text_translate_no} type="radio" name="text[translate]" value="0" {if !$_isTranslatorConfigured}disabled="disabled"{/if} /> {$lang.no}</label>

                        {if !$_isTranslatorConfigured}
                            <span class="field_description">{phrase key='translate_phrases_by_google_hint' db_check=true}</span>
                        {/if}
                    </td>
                </tr>
                </table>
    ...
    <div id="field_textarea" class="hide">
        ...
        <tr>
            <td class="name">{$lang.maxlength}</td>
            <td class="field">
                <input class="numeric" name="textarea[maxlength]" type="text" style="width: 50px; text-align: center;" value="{$textarea.maxlength}" maxlength="5" /> <span class="field_description">{$lang.default_textarea_value_des}</span>
            </td>
        </tr>
        </table>
        ...
        <div id="textarea_translate" {if !$sPost.textarea.multilingual}class="hide"{/if}>
            <table class="form">
            <tr>
                <td class="name">{$lang.translate_text}</td>
                <td class="field">
                    {if $sPost.textarea.translate == '1'}
                        {assign var='textarea_translate_yes' value='checked="checked"'}
                    {else}
                        {assign var='textarea_translate_no' value='checked="checked"'}
                    {/if}

                    <label><input {$textarea_translate_yes} type="radio" name="textarea[translate]" value="1" {if !$_isTranslatorConfigured}disabled="disabled"{/if} /> {$lang.yes}</label>
                    <label><input {$textarea_translate_no} type="radio" name="textarea[translate]" value="0" {if !$_isTranslatorConfigured}disabled="disabled"{/if} /> {$lang.no}</label>
    ...
    <div id="field_phone" class="hide">
        ...
        <tr>
            <td class="name">{$lang.messenger_icons}</td>
            <td class="field">
                {if $phone.messenger_icons == '1'}
                    {assign var='messenger_icons_yes' value='checked="checked"'}
                {elseif $phone.messenger_icons == '0'}
                    {assign var='messenger_icons_no' value='checked="checked"'}
                {else}
                    {assign var='messenger_icons_no' value='checked="checked"'}
                {/if}

                <label>
                    <input {$messenger_icons_yes} name="phone[messenger_icons]" type="radio" value="1" /> {$lang.yes}
                </label>
                <label>
                    <input {$messenger_icons_no}  name="phone[messenger_icons]" type="radio" value="0" /> {$lang.no}
                </label>
            </td>
        </tr>
    ...
    <!-- file storage field -->
    {assign var='file' value=$sPost.file}
    <div id="field_file" class="hide">
            ...
            <td class="field">
                <select name="file[type]">
                    ...
                    {foreach from=$l_file_types item='fTypes' key='ftKey'}
                        <option value="{$ftKey}" {if $ftKey == $file.type}selected="selected"{/if}>{$fTypes.name} ({$fTypes.ext})</option>
                    {/foreach}
                </select>
            </td>
        </tr>
        {if $grid_key == 'listingFieldsGrid'}
        <tr>
            <td class="name">{$lang.file_multipart_upload}</td>
            <td>
                {assign var='checkbox_field' value='multipart_upload'}

                {if $file.$checkbox_field == '1'}
                    {assign var=$checkbox_field|cat:'_yes' value='checked="checked"'}
                {elseif $sPost.$checkbox_field == '0'}
                    {assign var=$checkbox_field|cat:'_no' value='checked="checked"'}
                {else}
                ...
                <input {if $smarty.get.action == 'edit'}disabled="disabled"{/if} {$multipart_upload_yes} type="radio" id="{$checkbox_field}_yes" name="file[{$checkbox_field}]" value="1" /> <label for="{$checkbox_field}_yes">{$lang.yes}</label>
                <input {if $smarty.get.action == 'edit'}disabled="disabled"{/if} {$multipart_upload_no} type="radio" id="{$checkbox_field}_no" name="file[{$checkbox_field}]" value="0" /> <label for="{$checkbox_field}_no">{$lang.no}</label>

                {if $smarty.get.action == 'edit' && $file.$checkbox_field}
                    <input type="hidden" name="file[{$checkbox_field}]" value="1" />
                {/if}
            </td>
        </tr>
        {/if}
        ...
        <div class="file_limit_cont{if !$file.multipart_upload} hide{/if}">
            <table class="form">
            <tr>
                <td class="name"><span class="red">*</span>{$lang.files_limit}</td>
                <td>
                    <input class="numeric" name="file[limit]" type="text" style="width: 40px; text-align: center;" value="{$file.limit}" maxlength="2" />
                </td>
            </tr>
            </table>
        </div>
